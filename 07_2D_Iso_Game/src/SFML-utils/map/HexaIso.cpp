...
namespace sfutils
{
	namespace geometry
	{
		...
		HexaIso::__Initialiser HexaIso::__initialiser__;

		void HexaIso::init()
		{
			std::cout << "initialize shape in init()\n";
			...
			//std::cout << "size of HEXA ISO shape " << bounds.getSize().x << " " << bounds.getSize().y << "\n";
			//std::cout << "shape points and origin set\n";
		}
		...
		sf::IntRect HexaIso::getTextureRect(int x, int y, float scale)
		{
			...
			sf::IntRect res(pos.x, pos.y,
				height * scale,
				height / 2 * scale);
			//std::cout << res.top << " " << res.left << " " << res.width << " " << res.height << "IntRect for texture Rect\n";
			return res;
		}
