...
namespace book
{
	...
	bool Action::operator==(const sf::Event& event) const
	{
		...
		{
			if (m_type & Type::Released and m_event.type == sf::Event::EventType::KeyReleased)
				res = event.key.code == m_event.key.code;
		}
		break;
		...
		{
			if (m_type & Type::Released and m_event.type == sf::Event::EventType::MouseButtonReleased)
				res = event.mouseButton.button == m_event.mouseButton.button;
		}
		break;
		default: break;
