...
namespace book
{
    Game::Game(int X, int Y, int word_x, int word_y) : 
        ...
        m_window(sf::VideoMode(X, Y), "06_Multithreading"), 
        ...
        m_physicsThread(&Game::updatePhysics, this),
        m_isRunning(true)
    {
    ...
    void Game::run(int minimum_frames_per_second, int physics_frames_per_second)
    {
        ...
        m_physicsFramesPerSecond = physics_frames_per_second;
    ...
    {
        sf::Clock clock;
        const sf::Time timePerFrame = sf::seconds(1.f / m_physicsFramesPerSecond);
        static sf::Time timeSinceLastUpdate = sf::Time::Zero;
