...
namespace book
{
    ...
    {
        return m_initialLvl + m_nbLvl;
    }

    void Stats::setLevel(int lvl)
    {
        m_initialLvl = lvl;
        m_textLvl.setString("lvl : " + std::to_string(lvl));
    }
    ...
    void Stats::addLines(int lines)
    {
        if (lines > 0)
        {
            ...
            switch (lines)
            {
            case 1: m_nbScore += 40 * (m_initialLvl + m_nbLvl + 1); break;
            case 2: m_nbScore += 100 * (m_initialLvl + m_nbLvl + 1); break;
            case 3: m_nbScore += 300 * (m_initialLvl + m_nbLvl + 1); break;
            default: m_nbScore += 1200 * (m_initialLvl + m_nbLvl + 1); break;
            }

            m_textScore.setString("score : " + std::to_string(m_nbScore));

            m_nbLvl = m_initialLvl + (m_nbRows / 10);
