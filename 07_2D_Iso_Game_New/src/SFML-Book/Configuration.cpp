...
namespace book
{
    ...
    void Configuration::setGuiColor(const sf::Color& color)
    {
        ...
        sfutils::gui::Configuration::Colors::button_outline = sf::Color(color.r * 1.4, color.g * 1.4, color.b * 1.4, color.a);
        sfutils::gui::Configuration::Colors::label_text = sf::Color(color.r * 0.3, color.g * 0.3, color.b * 0.3, color.a);
    }
    ...
    {
        guiInputs.map(GuiInputs::Escape, sfutils::Action(sf::Keyboard::Key::Escape, sfutils::Action::Type::Pressed));
    }

    void Configuration::initEvents()
    {
        mapInputs.map(MapInputs::MoveUp, sfutils::Action(sf::Keyboard::Key::Up));
        mapInputs.map(MapInputs::MoveDown, sfutils::Action(sf::Keyboard::Key::Down));
        mapInputs.map(MapInputs::MoveLeft, sfutils::Action(sf::Keyboard::Key::Left));
        mapInputs.map(MapInputs::MoveRight, sfutils::Action(sf::Keyboard::Key::Right));
        mapInputs.map(MapInputs::TakeScreen, sfutils::Action(sf::Keyboard::Key::F1, sfutils::Action::Released));
    }
