...
namespace book
{
		...
		_entitiesLayer(new sfutils::Layer<MapComponent*>("Entity", 2))
	{
...

		{
			...
			std::shared_ptr<utils::json::Value> rootPtr = utils::json::Driver::parse(ss);
			...
			for (const utils::json::Value& value : spawn)
			{
				...
				{
					sf::Vector2i initial;
					initial.x = ob["x"].asInt();
					initial.y = ob["y"].asInt();
		...
		{
			// mouse layer where mouse light is which moves around with a cursor
		...
		case FuncIds::IdUpdateEntity:
		{
			...
			for (const packet::UpdateEntity::Data& data : event->getUpdates())
			{
				if (entities.isValid(data.entityId))
				{
					...
					sf::Vector2i oldCoord = _map->mapPixelToCoords(skin->_sprite.getPosition());

					skin->_sprite.setAnimation(skin->_animations.at(data.animationId));
					skin->_sprite.setPosition(data.position);

					setPosition(entities.get(data.entityId), oldCoord, data.coord);
					hp->_hp = data.hp;
				}
			}
		} break;
		...
		case FuncIds::IdOnHitEntity:
		{
			...
			for (const packet::OnHitEntity::Data& data : event->getHit())
			{
				if (entities.isValid(data.entityId))
				{
					...
					{
						Entity& enemy = entities.get(data.enemyId);
						e.onHit(e, _map->mapPixelToCoords(e.getPosition()), enemy, _map->mapPixelToCoords(enemy.getPosition()), *this);
					}
				}
			}
		} break;
