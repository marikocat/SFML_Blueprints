...
namespace sfutils
{
	namespace map
	{
		...
		MapViewer::MapViewer(sf::RenderWindow& window, const VMap& map, const ActionMap<int>& action_map) : ActionTarget(action_map),
			_map(map), _zoom(1), _moveX(0), _moveY(0), _movementSpeed(5), _window(window)
		{
			bind(Action(sf::Event::EventType::MouseWheelMoved), [this](const sf::Event& event)
				{
