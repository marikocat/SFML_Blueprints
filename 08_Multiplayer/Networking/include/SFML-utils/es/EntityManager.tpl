#include <SFML-utils/es/Component.h>
...
namespace sfutils
{
	namespace es
	{
		...
		template<class ENTITY>
		void EntityManager<ENTITY>::remove(std::uint32_t id)
		{
		...
		void EntityManager<ENTITY>::removeComponent(std::uint32_t id)
		{
			...
			_entitiesComponentsMask.at(id).reset(family);
		}
		...
		template<class ENTITY>
		inline void EntityManager<ENTITY>::reset(std::uint32_t id)
		{
			ENTITY* e = _entitiesAllocated.at(id);
		...
		template<typename ... COMPONENT>
		EntityManager<ENTITY>::View<COMPONENT...>::View(EntityManager<ENTITY>& manager,
			const std::bitset<MAX_COMPONENTS>& mask, ComponentHandle<COMPONENT, ENTITY>& ... components)
			: _manager(manager), _mask(mask), _handles(std::tuple<ComponentHandle<COMPONENT, ENTITY>&...>(components ...))
		{
		...
		template<typename ENTITY>
		template<typename ... COMPONENT>
		EntityManager<ENTITY>::View<COMPONENT...>::iterator::iterator(View& view,
			EntityManager<ENTITY>::container::iterator it, EntityManager<ENTITY>::container::iterator it_end)
			: _view(view), _it(it), _itEnd(it_end)
		{
		}
		...
		template<typename ENTITY>
		template<typename ... COMPONENT>
		bool EntityManager<ENTITY>::View<COMPONENT...>::iterator::operator==(const iterator& other)
		{
