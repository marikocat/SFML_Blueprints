	...
	namespace packet
	{
		...
		sf::Packet& operator>>(sf::Packet& packet, SetListGame& self)
		{
			...
			for (unsigned int i = 0; i < size; ++i)
			{
				...
				SetListGame::Game game = { teams, players, id };
		...
		JoinGameConfirmation::JoinGameConfirmation(const std::string& mapDatas, int team) : NetworkEvent(FuncIds::IdJoinGameConfirmation),
			_mapDatas(mapDatas), _teamId(team)
		{
		}
		...
		const void JoinGameConfirmation::addTeam(Data&& data)
		{
		...
		int JoinGameReject::gameId() const
		{
			return _gameId;
		}
		...
		RequestCreateEntity::RequestCreateEntity(short int type, const sf::Vector2i& coord) : NetworkEvent(FuncIds::IdRequestCreateEntity),
			_entityType(type), _coord(coord)
		{
		}
		...
		void CreateEntity::add(Data&& update)
		{
		...
		void UpdateEntity::add(Data&& update)
		{
			_updates.emplace_back(update);
		}
		...
		void OnHittedEntity::add(Data&& data)
		{
		...
		sf::Packet& operator>>(sf::Packet& packet, OnHittedEntity& self)
		{
			...
			for (unsigned int i = 0; i < size; ++i)
			{
				...
				data.enemyCoord.x = ex;
				data.enemyCoord.y = ey;
		...
		sf::Packet& operator<<(sf::Packet& packet, const OnHittedEntity& self)
		{
			...
			for (const OnHittedEntity::Data& data : self._updates)
			{
				packet << sf::Uint32(data.entityId)
					...
					<< sf::Int32(data.enemyCoord.x)
					<< sf::Int32(data.enemyCoord.y);
			}
			return packet;
		}
		...
		void OnHitEntity::add(Data&& data)
		{
			_updates.emplace_back(data);
		}
