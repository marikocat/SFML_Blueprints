#include <SFML-utils/gui/Frame.h>
...
namespace sfutils
{
	namespace gui
	{
		...
		Frame::Frame(sf::RenderWindow& window, const ActionMap<int>& map) : Container(nullptr), ActionTarget(map),
			_window(window), _view(_window.getDefaultView())
		{
			ActionTarget::bind(Action(sf::Event::EventType::Resized), [this](const sf::Event& event)
				{
